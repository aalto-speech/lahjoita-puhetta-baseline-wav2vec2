...
def main():
    ...
    model = Wav2Vec2ForCTC.from_pretrained(model_args.model_name_or_path, cache_dir=model_args.cache_dir, gradient_checkpointing=training_args.gradient_checkpointing)
    orthography = Orthography.from_name(data_args.orthography.lower())
    processor = orthography.create_processor(model_args)

    train_dataset = datasets.load_dataset('csv', data_files='/scratch/work/getmany1/wav2vec/donatespeech_train_100h_50s.csv', split='train', cache_dir=model_args.cache_dir)
    val_dataset = datasets.load_dataset('csv', data_files='/scratch/work/getmany1/wav2vec/donatespeech_dev_50s.csv', split='train', cache_dir=model_args.cache_dir)
    ...
    def prepare_example(example):
        ...
        if "segment_start_time" in example:
            example["speech"] = example["speech"][int(example["segment_start_time"]*example["sampling_rate"]):
    ...
    train_dataset = train_dataset.map(prepare_example, num_proc=data_args.preprocessing_num_workers, remove_columns=["file"])
    val_dataset = val_dataset.map(prepare_example, num_proc=data_args.preprocessing_num_workers, remove_columns=["file"])
